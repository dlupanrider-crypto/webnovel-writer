---
name: webnovel-init
description: Use when starting a new web novel project or initializing creative constraints. Collects genre, characters, worldbuilding, and plot information to generate project skeleton files.
version: 5.5.4
metadata:
  openclaw:
    requires:
...
- `templates/...` 相对项目根目录（`${SKILL_ROOT}/../templates/...`）。
- `../templates/...` 指向共享模板目录。
...
- 禁止在 skill 目录下生成项目文件（`${SKILL_ROOT}`）。
